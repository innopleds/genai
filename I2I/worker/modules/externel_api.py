...
# async def check_image_safety(image, safety_checker, feature_extractor):
#     # 이미지에서 특징 추출
#     clip_input = feature_extractor(images=[image], return_tensors="pt").pixel_values
#     # 안전성 검사를 위한 CLIP 모델 예측
#     outputs = safety_checker(pixel_values=clip_input)
#     logits_per_image = outputs.logits_per_image  # 이미지에 대한 로짓 값
#     probs = logits_per_image.softmax(dim=1)  # 소프트맥스를 사용해 확률로 변환

#     # 위험성이 높은 클래스를 정의할 수 있음 (예: 특정 임계값을 넘는 경우 비안전으로 간주)
#     unsafe_threshold = 0.5  # 예시 임계값
#     is_safe = probs[:, 0].item() < unsafe_threshold  # 클래스 0은 안전한 클래스로 가정
#     return is_safe
